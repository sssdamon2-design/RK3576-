    ...
    char *argv[])
{
    ...
    /* =====================================================
     * 5. 查询 Input / Output Tensor 属性
     *
     * input_attr:
     *   后面给 rknn_set_io_mem() 使用。
     *
     * output_attrs:
     *   仍然沿用普通 rknn_outputs_get() 路径，
     *   这样本实验只改变“输入路径”一个变量。
     * ===================================================== */
    ...
    /* =====================================================
     * 6. 使用 RKNN I/O Memory 设置输入
     *
     * 本实验只改变输入 Buffer 的提交方式。
     *
     * 输入仍然是：
     *   640 x 640 x 3
     *   RGB UINT8
    ...
    /*
     * 第一版沿用 Rockchip create_mem 示例的做法：
     * 使用查询得到的 size_with_stride 分配。
     *
     * 对当前 FP16 模型：
     * size_with_stride = 2457600 bytes。
     *
     * 实际复制的 UINT8 RGB 输入是：
